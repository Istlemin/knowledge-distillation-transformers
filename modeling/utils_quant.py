...
class QuantizeLinear(nn.Linear):
    ...
    def __init__(self,  *kargs,bias=True, config = None):
        ...
        self.register_buffer('weight_clip_val', torch.tensor([-2.5, 2.5]))
        if self.quantize_act:
            self.input_bits = 8
            self.act_quantizer = AsymQuantizer
            self.register_buffer('act_clip_val', torch.tensor([-2.5, 2.5]))
...
if __name__=="__main__":
    ...
    layer = QuantizeLinear(16,16,config=Namespace(quantize_act=True,weight_bits = 2,
                                                input_bits = 8,
                                                clip_val = 2.5))
    torch.manual_seed(0)
